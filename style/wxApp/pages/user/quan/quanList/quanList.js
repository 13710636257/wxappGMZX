...
Page({
  ...
  getList: function (e) {
    var that = this;
    http.httpGet(config.HTTP_wxfans_quanList_URL, { page: that.data.page, rows: that.data.rows }, function (res) {
      if (res.success) {
        /*判断有没有数据*/
        if (res.obj.total == 0) {
          that.setData({
  ...
  tapAction: function (e) {
    console.log(e);
    wx.navigateTo({
      url: '../quanDetail/quanDetail?id=' + e.currentTarget.dataset.id,
      success: function (res) {
        // success
      },
      fail: function () {
